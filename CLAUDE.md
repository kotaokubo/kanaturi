...
https://github.com/kotaokubo/kanaturi は public リポジトリで、コード、コミットメッセージ、
PR の本文、`.claude/skills/` の中身まで誰でも読める。GitHub Pages の無料プランで
組合サイトを配信しているため、private にはできない。

次のものはコミットしない。PR 本文やコミットメッセージにも書かない。

- 関係者のメールアドレス、電話番号（サイトに掲載する組合の連絡先を除く）
- 関係者との個別のやり取りの内容や引用
- 相手方の意向や事情についての記述、こちらの読み
- 見積もり、費用、営業上の見立て

活動報告に載せる写真と氏名は、本人の了承を得たうえで公開している。それ以外の情報は
Claude Code のメモリ（`~/.claude/projects/` 配下、リポジトリ外）に置く。
判断に迷うときは、組合サイトの読者に見せてよい情報かで切り分ける。
